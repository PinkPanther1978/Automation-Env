...
Expected pattern: /.*trace-viewer/
Received string:  "https://playwright.dev/"
Timeout: 5000ms

Call log:
  - Expect "toHaveURL" with timeout 5000ms
    13 × locator resolved to <html lang="en" dir="ltr" data-theme="light" data-has-hydrated="true" data-theme-choice="system" class="plugin-pages plugin-id-default" data-rh="lang,dir,class,data-has-hydrated">…</html>
...
```ts
  ...
  14 | 
  15 |   // 4. Press Enter and verify the browser successfully navigated to the Trace Viewer page
  16 |   await searchInput.press('Enter');
> 17 |   await expect(page).toHaveURL(/.*trace-viewer/);
     |                      ^ Error: expect(page).toHaveURL(expected) failed
  18 | 
  19 |   // 5. Let Playwright handle the screenshot safely
  20 |   await page.screenshot({ path: 'screenshot.png', fullPage: true });
  21 | });
```